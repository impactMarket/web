...
const LogoList2 = (props: LogoListSliceType) => {
    const labels = (props?.primary?.labels || '')
        .split(',')
        .map((string) => string.trim());
    const items = labels.map((label, index) => ({
        items: props.items
            .filter(
                ({ labelIndex }) => labelIndex.toString() === index.toString()
            )
    ...
    return (
        <Section
            id={props?.primary?.id?.substring(1)}
            sPadding={{ sm: '3 null', xs: '2 null' }}
        >
            <Grid>
                <Row>
                    <Col center xs={12}>
                        <Div
                            inlineFlex
                            sAlignItems="center"
                            sFlexDirection={{ md: 'row', xs: 'column' }}
                        >
